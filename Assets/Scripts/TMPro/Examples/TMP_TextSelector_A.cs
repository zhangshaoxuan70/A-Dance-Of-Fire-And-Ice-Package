...
namespace TMPro.Examples
{
	public class TMP_TextSelector_A : MonoBehaviour, IPointerEnterHandler, IEventSystemHandler, IPointerExitHandler
	{
		...
		private void LateUpdate()
		{
			...
			int num = TMP_TextUtilities.FindIntersectingCharacter(m_TextMeshPro, UnityEngine.Input.mousePosition, Camera.main, visibleOnly: true);
			if (num != -1 && num != m_lastCharIndex && (UnityEngine.Input.GetKey(KeyCode.LeftShift) || UnityEngine.Input.GetKey(KeyCode.RightShift)))
			{
				m_lastCharIndex = num;
			...
			int num2 = TMP_TextUtilities.FindIntersectingLink(m_TextMeshPro, UnityEngine.Input.mousePosition, m_Camera);
			...
			if (num2 != -1 && num2 != m_selectedLink)
			{
				...
				RectTransformUtility.ScreenPointToWorldPointInRectangle(m_TextMeshPro.rectTransform, UnityEngine.Input.mousePosition, m_Camera, out Vector3 _);
			...
			int num3 = TMP_TextUtilities.FindIntersectingWord(m_TextMeshPro, UnityEngine.Input.mousePosition, Camera.main);
