...
public class Cutscene5d : TaroCutsceneScript
{
	...
	{
		base.Awake();
		shouldDisplay = (!GCS.seenCutscene4_12 && Persistence.GetTaroStoryProgress() < 4);
		...
		if (shouldDisplay)
		{
			...
			dialog.Add(RDString.Get("neoCosmosStory.T4.12.1"));
			dialog.Add(RDString.Get("neoCosmosStory.T4.12.2"));
			dialog.Add(RDString.Get("neoCosmosStory.T4.12.3"));
			dialog.Add(RDString.Get("neoCosmosStory.T4.12.4"));
		}
