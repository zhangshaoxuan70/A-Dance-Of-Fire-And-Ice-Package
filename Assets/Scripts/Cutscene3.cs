...
public class Cutscene3 : TaroCutsceneScript
{
	...
	{
		base.Awake();
		shouldDisplay = (!GCS.seenCutscene2_1b && Persistence.GetTaroStoryProgress() < 2);
		...
		if (shouldDisplay)
		{
			...
			dialog.Add(RDString.Get("neoCosmosStory.T2.3.1"));
			dialog.Add(RDString.Get("neoCosmosStory.T2.3.2"));
			dialog.Add(RDString.Get("neoCosmosStory.T2.3.3"));
			dialog.Add(RDString.Get("neoCosmosStory.T2.3.4"));
			dialog.Add(RDString.Get("neoCosmosStory.T2.3.5"));
		}
	...
	private new void Update()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			midspins[i].transform.eulerAngles = Vector3.forward * (90f + 5f * Mathf.Sin(Time.time * 0.7f * MathF.PI + (float)i * 0.5f));
		}
		base.Update();
	}
}
