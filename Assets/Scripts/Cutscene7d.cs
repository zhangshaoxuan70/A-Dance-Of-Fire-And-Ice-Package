...
public class Cutscene7d : TaroCutsceneScript
{
	...
	{
		base.Awake();
		shouldDisplay = (!GCS.seenCutscene5_x && Persistence.GetTaroStoryProgress() < 6);
		if (GCS.enableCutsceneT5 && !GCS.seenCutscene5_x)
		{
			shouldDisplay = true;
		}
		if (RDC.skipCutscenes || GCS.practiceMode || GCS.speedTrialMode)
		{
			shouldDisplay = false;
		}
		...
		if (shouldDisplay)
		{
			...
			dialog.Add(RDString.Get("neoCosmosStory.T5.X.1"));
			dialog.Add(RDString.Get("neoCosmosStory.T5.X.2"));
			dialog.Add(RDString.Get("neoCosmosStory.T5.X.3"));
			dialog.Add(RDString.Get("neoCosmosStory.T5.X.4"));
			dialog.Add(RDString.Get("neoCosmosStory.T5.X.5"));
		}
