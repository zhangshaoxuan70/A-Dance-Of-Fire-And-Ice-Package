...
public class Cutscene6 : TaroCutsceneScript
{
	...
	{
		base.Awake();
		shouldDisplay = (!GCS.seenCutsceneP_1 && Persistence.GetTaroStoryProgress() < 5);
		...
		if (shouldDisplay)
		{
			...
			dialog.Add(RDString.Get("neoCosmosStory.TP.1.1"));
			dialog.Add(RDString.Get("neoCosmosStory.TP.1.2"));
			dialog.Add(RDString.Get("neoCosmosStory.TP.1.3"));
			dialog.Add(RDString.Get("neoCosmosStory.TP.1.4"));
		}
