...
public class Cutscene5a : TaroCutsceneScript
{
	...
	{
		base.Awake();
		shouldDisplay = (!GCS.seenCutscene4_2 && Persistence.GetTaroStoryProgress() < 4);
		if (RDC.skipCutscenes)
		{
			shouldDisplay = false;
		}
		printe($"seen scene 4_2: {GCS.seenCutscene4_2}, taro story prog: {Persistence.GetTaroStoryProgress()}, should display: {shouldDisplay}");
		...
		if (shouldDisplay)
		{
			...
			dialog.Add(RDString.Get("neoCosmosStory.T4.2.1"));
			dialog.Add(RDString.Get("neoCosmosStory.T4.2.2"));
			dialog.Add(RDString.Get("neoCosmosStory.T4.2.3"));
			dialog.Add(RDString.Get("neoCosmosStory.T4.2.4"));
		}
