...
public class Cutscene5e : TaroCutsceneScript
{
	...
	{
		base.Awake();
		shouldDisplay = (!GCS.seenCutscene4b_1 && !Persistence.IsWorldComplete("T4EX"));
		...
		if (shouldDisplay)
		{
			...
			dialog.Add(RDString.Get("neoCosmosStory.T4EX.1.1"));
			dialog.Add(RDString.Get("neoCosmosStory.T4EX.1.2"));
			dialog.Add(RDString.Get("neoCosmosStory.T4EX.1.3"));
			dialog.Add(RDString.Get("neoCosmosStory.T4EX.1.4"));
		}
