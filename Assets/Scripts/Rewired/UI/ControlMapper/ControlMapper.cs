...
namespace Rewired.UI.ControlMapper
{
	...
	public class ControlMapper : MonoBehaviour
	{
		...
		private void OnJoystickPreDisconnect(ControllerStatusChangedEventArgs args)
		{
			if (initialized)
			{
				bool showController = _showControllers;
			}
		}
		...
		private void DrawPlayersGroup()
		{
			...
			for (int i = 0; i < playerCount; i++)
			{
				...
				if (player != null)
				{
					GUIButton gUIButton = new GUIButton(UITools.InstantiateGUIObject<ButtonInfo>(prefabs.button, references.playersGroup.content, "Player" + i.ToString() + "Button"));
		...
		private void SubscribeRewiredInputEventAllPlayers(int actionId, Action<InputActionEventData> callback)
		{
			if (actionId >= 0 && callback != null)
			{
				...
				{
					UnityEngine.Debug.LogWarning("Rewired Control Mapper: " + actionId.ToString() + " is not a valid Action id!");
				}
				else
				{
