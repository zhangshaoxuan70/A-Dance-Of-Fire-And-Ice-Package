	...
	public class ControlRemappingDemo1 : MonoBehaviour
	{
		...
		private void DrawControllerSelector()
		{
			if (selectedPlayer != null)
			{
				...
				if (GUILayout.Toggle(flag, "Keyboard", "Button", GUILayout.ExpandWidth(expand: false)) != flag)
				{
					selectedController.Set(0, ControllerType.Keyboard);
					ControllerSelectionChanged();
				}
				...
				if (GUILayout.Toggle(flag, "Mouse", "Button", GUILayout.ExpandWidth(expand: false)) != flag)
				{
				...
				foreach (Joystick joystick in selectedPlayer.controllers.Joysticks)
				{
					...
					if (GUILayout.Toggle(flag, joystick.name, "Button", GUILayout.ExpandWidth(expand: false)) != flag)
					{
		...
		private void DrawMapCategories()
		{
			if (selectedPlayer != null && selectedController.hasSelection)
			{
				...
				foreach (InputMapCategory userAssignableMapCategory in ReInput.mapping.UserAssignableMapCategories)
				{
					...
					if (GUILayout.Toggle(flag, (userAssignableMapCategory.descriptiveName != string.Empty) ? userAssignableMapCategory.descriptiveName : userAssignableMapCategory.name, "Button", GUILayout.ExpandWidth(expand: false)) != flag)
					{
						selectedMapCategoryId = userAssignableMapCategory.id;
						selectedMap = selectedPlayer.controllers.maps.GetFirstMapInCategory(selectedController.type, selectedController.id, userAssignableMapCategory.id);
					}
		...
		private bool ProcessElementAssignmentConflictCheck(ElementAssignmentChange entry)
		{
			...
			if (conflictFoundEventData.isProtected)
			{
				...
				DialogHelper dialogHelper = dialog;
				int id = entry.id;
				...
				dialogHelper.StartModal(id, DialogHelper.DialogType.AssignElement, windowProperties, DialogResultCallback);
			}
			else
			{
				...
				DialogHelper dialogHelper2 = dialog;
				int id2 = entry.id;
				windowProperties = new WindowProperties
				{
					title = "Assignment Conflict",
				...
				dialogHelper2.StartModal(id2, DialogHelper.DialogType.AssignElement, windowProperties, DialogResultCallback);
			}
			return false;
		}
