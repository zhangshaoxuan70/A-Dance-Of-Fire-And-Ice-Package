	...
	public class UserDataStore_PlayerPrefs : UserDataStore
	{
		...
		private class JoystickAssignmentHistoryInfo
		{
			...
			public JoystickAssignmentHistoryInfo(Joystick joystick, int oldJoystickId)
			{
				if (joystick == null)
				{
					throw new System.ArgumentNullException("joystick");
				}
		...
		{
			return GetBasePlayerPrefsKey(player) + "|dataType=ControllerMap" + GetControllerMapPlayerPrefsKeyCommonSuffix(player, controllerIdentifier, categoryId, layoutId, ppKeyVersion);
		}
		...
		{
			return GetBasePlayerPrefsKey(player) + "|dataType=ControllerMap_KnownActionIds" + GetControllerMapPlayerPrefsKeyCommonSuffix(player, controllerIdentifier, categoryId, layoutId, ppKeyVersion);
		}
		...
		private static string GetControllerMapPlayerPrefsKeyCommonSuffix(Player player, ControllerIdentifier controllerIdentifier, int categoryId, int layoutId, int ppKeyVersion)
		{
			...
			if (ppKeyVersion >= 2)
			{
				str = str + "|kv=" + ppKeyVersion.ToString();
			}
			str = str + "|controllerMapType=" + GetControllerMapType(controllerIdentifier.controllerType).Name;
			str = str + "|categoryId=" + categoryId.ToString() + "|layoutId=" + layoutId.ToString();
			if (ppKeyVersion >= 2)
			{
				str = str + "|hardwareGuid=" + controllerIdentifier.hardwareTypeGuid.ToString();
				if (controllerIdentifier.hardwareTypeGuid == Guid.Empty)
				{
					str = str + "|hardwareIdentifier=" + controllerIdentifier.hardwareIdentifier;
				}
				if (controllerIdentifier.controllerType == ControllerType.Joystick)
				{
					str = str + "|duplicate=" + GetDuplicateIndex(player, controllerIdentifier).ToString();
				}
			}
			else
			{
		...
		{
			return playerPrefsKeyPrefix + "|dataType=CalibrationMap" + "|controllerType=" + joystick.type.ToString() + "|hardwareIdentifier=" + joystick.hardwareIdentifier + "|hardwareGuid=" + joystick.hardwareTypeGuid.ToString();
		}

		private string GetInputBehaviorPlayerPrefsKey(Player player, int inputBehaviorId)
		{
			return GetBasePlayerPrefsKey(player) + "|dataType=InputBehavior" + "|id=" + inputBehaviorId.ToString();
		}
		...
		private static Type GetControllerMapType(ControllerType controllerType)
		{
			...
			default:
				UnityEngine.Debug.LogWarning("Rewired: Unknown ControllerType " + controllerType.ToString());
				return null;
			}
		}
	}
