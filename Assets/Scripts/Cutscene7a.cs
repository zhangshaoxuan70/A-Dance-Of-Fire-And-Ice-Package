...
public class Cutscene7a : TaroCutsceneScript
{
	...
	{
		base.Awake();
		shouldDisplay = (!GCS.seenCutscene5_1 && Persistence.GetTaroStoryProgress() < 6);
		...
		if (shouldDisplay)
		{
			...
			dialog.Add(RDString.Get("neoCosmosStory.T5.1.1"));
			dialog.Add(RDString.Get("neoCosmosStory.T5.1.2"));
			dialog.Add(RDString.Get("neoCosmosStory.T5.1.3"));
		}
